...
def audit_payload(payload: dict) -> dict:
    ...
    injection_payload = int(canonical_input.get("injection_payload_bitmask", 0))
    sensitive_surface = int(canonical_input.get("sensitive_surface_bitmask", 0))
    suspicious_request = int(canonical_input.get("target_suspicious_request_bitmask", 0))
    has_scanner_fingerprint = bool(canonical_input.get("has_scanner_fingerprint", False))
    has_malformed_encoding = bool(canonical_input.get("has_malformed_encoding", False))
    risk_score = float(canonical_input.get("risk_score", 0.0))
    likely_benign = bool(canonical_input.get("likely_benign_request", False))
    allow = bool(
        canonical_input.get(
            "allow",
            (
